...
def enviar(funcion_datos,canal=15,poder=nr.POWER_0, velocidad=nr.SPEED_250K,espera=500):
    ...
    while fails < max_fails:
        ...
        # 1. Recolectar datos
        try:
            (ax, ay, az, gx, gy, gz, temp_mpu, temp_bmp, presion, temp_dht, hum, lat, lon, sats, speed) = funcion_datos()
        ...
        try:
            ...
            counter += 1 # Incrementar incluso si falla el empaquetado para no estancarse
            utime.sleep_ms(1000)
            continue
        # 4. Enviar el paquete
